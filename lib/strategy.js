...
function detectVolumeDivergence(candles, deltaSum, priceLookback) {
  return candles.map((c, i) => {
    if (i < priceLookback) return null;
    const priceUp = c.close > candles[i - priceLookback].close;
    const priceDown = c.close < candles[i - priceLookback].close;
    if (priceUp && deltaSum[i] <= 0) return 'bearish';
    if (priceDown && deltaSum[i] >= 0) return 'bullish';
    return null;
  });
}
