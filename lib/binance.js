...
// Futures endpoint (fapi), not spot — funding rate is a perpetuals-only
// concept. Full history back to symbol listing (~2020 for BTCUSDT), unlike
// openInterestHist/globalLongShortAccountRatio which the API only serves for
// the last ~30 days (confirmed by hand: a startTime older than that returns
// error -1130 "parameter startTime is invalid").
async function fetchFundingRateHistory(symbol, startTime, endTime, onProgress) {
  let all = [];
  let curEnd = endTime;
  let batches = 0;
  while (curEnd > startTime) {
    const url = `https://fapi.binance.com/fapi/v1/fundingRate?symbol=${symbol}&endTime=${curEnd}&limit=1000`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`Binance funding rate fetch failed (${res.status})`);
    const raw = await res.json();
    if (!raw.length) break;
    const batch = raw.map(f => ({ time: f.fundingTime, rate: parseFloat(f.fundingRate) }));
    all = batch.concat(all);
    batches++;
    if (onProgress) onProgress(batches);
    if (batch[0].time <= startTime) break;
    curEnd = batch[0].time - 1;
    await new Promise(r => setTimeout(r, 120));
  }
  return all.filter(f => f.time >= startTime && f.time <= endTime);
}
...
module.exports = { fetchKlines, fetchKlinesPaged, fetchFundingRateHistory, generateMockCandles };
