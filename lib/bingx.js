...
// Reduce-only counterpart to placeLimitEntry — used for AlitoBot's full-close
// exits (take-profit / circuit breaker). Always LIMIT, never MARKET: per
// explicit instruction, every AlitoBot order (adds and exits alike) must be a
...
// cap worst-case slippage. See worker/runAlitobot.js for how the price is
...
// Must be called once before AlitoBot opens a cycle — nothing in this file
...
// This is the "a margen" tranche destination in the AlitoBot rules, distinct
...
// Live floating ROI% for one open position — AlitoBot's tick loop polls this
...
// real populated position the first time a AlitoBot cycle actually opens one
...
async function getPositionRoiPct({ symbol, positionSide }) {
  ...
  return {
    ...
    liquidationPrice: pos.liquidationPrice != null ? Number(pos.liquidationPrice) : null,
    raw: pos,
  };
}

// Real trading costs (commission, funding fee, realized PnL) since a given
// timestamp — used to compute a true net PnL (gross ± what BingX actually
// charged), not just the raw price-driven unrealized number. Field names
// (incomeType, income) match BingX's documented
// /openApi/swap/v2/user/income response — like getPositionRoiPct, confirm
// against a real populated response the first time this runs against a live
// account with actual fills and adjust here if it differs.
async function getIncomeHistory({ symbol, startTime, incomeType, limit = 1000 }) {
  const params = { symbol, limit };
  if (startTime) params.startTime = startTime;
  if (incomeType) params.incomeType = incomeType;
  return signedRequest('GET', '/openApi/swap/v2/user/income', params);
}
