// Server-only (uses fs) — persists AlitoBot's cycle state across worker
// restarts. Same file-backed pattern as lib/dailyLossTracker.js /
// lib/modeStore.js. BingX is still the source of truth for what's actually
// open (see worker/runAlitobot.js's recoverState) — this file is the *local
// bookkeeping* of cargadores/balas committed and which calendar day the
// last daily recarga check ran, which BingX itself has no concept of.
const fs = require('fs');
...
function idleState() {
  return {
    ...
    totalQuantity: 0,
    entryOrderIds: [],
    lastRecargaDay: null, // 'YYYY-MM-DD' — recargas (bala tranches) run at most once per calendar day
...
class AlitobotStateStore {
  ...
  // (worker/runAlitobot.js) owns the actual decision logic
  // (lib/alitobotRules.js mutates a working copy of `state`), this just
  // commits it to disk.
  update(partial) {
...
module.exports = { AlitobotStateStore, idleState };
